...
chrome.runtime.onInstalled.addListener(() => {
  console.log('Crop Disease Analyzer extension installed');
  
  // Create context menu
  chrome.contextMenus.create({
    id: "analyzeImage",
    title: "Analyze this image with Crop Analyzer",
    contexts: ["image"]
  });
});
...
async function processImageForAnalysis(imageUrl) {
  try {
    ...
    const modelResponse = await fetch('http://127.0.0.1:5001/predict', {
      method: 'POST',
      body: formData
    });
