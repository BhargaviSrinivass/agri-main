...
# NOTE: The token remains the same as it is tied to your Telegram bot instance.
BOT_TOKEN = "8293034588:AAHAKkL6XZMKgjqwt79gFxk7R8E0lVZHt8I" 

# 🚨 CHANGE: This must now match the host and port for your livestock model (http://0.0.0.0:5002)
FLASK_API_URL = "http://127.0.0.1:5002/predict" 
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # 🐄 CHANGE: Updated welcome message for livestock
    await update.message.reply_html(
        f"Hi {user.mention_html()}! 👋\n\nI am a **livestock disease detection bot**. Please send me a clear picture of an animal, and I will try to predict if it has a disease and recommend a treatment.",
    )
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # 3. Download the file content as bytearray and convert to bytes
        ...
        if prediction_data['success']:
            ...
            if detection['detected']:
                # 🐄 CHANGE: Updated title for livestock
                result_message = (
                ...
                # 🐄 CHANGE: Updated status message for livestock
                result_message = (
                    f"**✅ Animal Status: Healthy ✅**\n\n"
                    f"**Confidence:** {confidence_str}\n\n"
                    f"**Recommendation:**\n"
                    f"_{treatment}_"
                )
        ...
        # 🚨 CHANGE: Updated port reference for error message
        await update.message.reply_text("⚠️ **Error:** Could not connect to the ML service. Please ensure the Flask API is running at the correct address (http://127.0.0.1:5002).")
